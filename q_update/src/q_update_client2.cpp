...
void select_action_flag_callback(std_msgs::Int64 msg){
...
int epsilon_greedy(int epsilon, int s, int a_num, float Qtable[num_s][num_a]){
	int a;
	// printf("rand() = %d\n", rand()%100);
	if(epsilon>rand()%100){
		a = rand()%a_num;
...
int main(int argc, char *argv[]){
	...
	FILE *fpq;
	FILE *fpqs;
	const char *fname = "/home/amsl/ros_catkin_ws/src/arm_q_learning/test_results/ql_test3.txt";
	...
	ros::Subscriber sub_4 = n.subscribe("/select_action_flag", 10, select_action_flag_callback);
	...
	while(ros::ok()){
		...
		/*	if((step_count%100)==0){
				sprintf(fqsname, "/home/amsl/ros_catkin_ws/src/arm_q_learning/q_table/q_table3_step_%d.txt", step_count);
				fpqs = fopen(fqsname, "a");
				if(fpqs == NULL){
					printf("%sファイルが開けません\n", fqsname);
					return -1;
				}
			...
			if(reward >= 6){
				...
				if(episode_count <= 1000){
					if((episode_count%20)==0){
						sprintf(fqname, "/home/amsl/ros_catkin_ws/src/arm_q_learning/q_table/q_table3_%d.txt", episode_count);
						fpq = fopen(fqname, "a");
						if(fpq == NULL){
							printf("%sファイルが開けません\n", fqname);
							return -1;
