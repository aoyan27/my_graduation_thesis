...
class agent:
	...
	def epsilon_greedy(self, joint1, joint3, joint5):
        ...
        def main(self):
            ...
            target_end_x = np.array([])
            target_end_y = np.array([])
            target_end_z = np.array([])
                ...
                if self.wait_flag:
                    ...
                    if count == 100:
                        ...
                        if step_count == 0:
                            ...
                            plot_optimal_x.append(list(self.optimal_path_x))
                            #  print plot_optimal_x
                            plot_optimal_y.append(list(self.optimal_path_y))
                            plot_optimal_z.append(list(self.optimal_path_z))

                            self.optimal_path_x = np.array([])
                            self.optimal_path_y = np.array([])
                            self.optimal_path_z = np.array([])

                            #  target_end_x = np.append(target_end_x, np.array([self.target_point.points[0].x]))
                            #  target_end_y = np.append(target_end_y, np.array([self.target_point.points[0].y]))
                            #  target_end_z = np.append(target_end_z, np.array([self.target_point.points[0].z]))
                        pub_11_flag = False
                        pub_13_flag = False
                    ...
                    if self.mode_dqn_flag:
                        ...
                        if self.reward >= 1:
                            ...
                            plot_x.append(list(xs))
                            plot_y.append(list(ys))
                            plot_z.append(list(zs))
                            ...
                            #  rand_target_y = self.target_init_y
                            rand_target_y += math.fabs(2*self.target_init_y) / (10.0 - 1.0)
            ...
            print len(plot_x)
            print "x[0]", len(plot_x[0])
            print "y[0]", len(plot_y[0])
            print "z[0]", len(plot_z[0])

            for i in range(len(plot_x)):
                x_numpy = np.array(plot_x[i])
                y_numpy = np.array(plot_y[i])
                z_numpy = np.array(plot_z[i])
                
                init_arm_x = np.array(plot_x[i][0])
                init_arm_y = np.array(plot_y[i][0])
                init_arm_z = np.array(plot_z[i][0])


                target_end_x = np.array(plot_x[i][len(plot_x[i])-1])
                target_end_y = np.array(plot_y[i][len(plot_y[i])-1])
                target_end_z = np.array(plot_z[i][len(plot_z[i])-1])
                
                #  target_end_x = np.append(target_end_x, np.array(plot_x[i][len(plot_x[i])-1]))
                #  target_end_y = np.append(target_end_y, np.array(plot_y[i][len(plot_y[i])-1]))
                #  target_end_z = np.append(target_end_z, np.array(plot_z[i][len(plot_z[i])-1]))

                print "target_end_x : ", target_end_x
                print "target_end_y : ", target_end_y
                print "target_end_z : ", target_end_z

                ax.plot_wireframe(x_numpy, y_numpy, z_numpy, color=(1.0, 0.0, 0.0))

                ax.scatter3D(init_arm_x, init_arm_y, init_arm_z, color=(0.0, 0.0, 0.0), marker="s", s=50)
                
                ax.scatter3D(target_end_x, target_end_y, target_end_z, color=(0.0, 0.0, 0.0), marker="o", s=50)
                #  ax.scatter3D(target_end_x[i], target_end_y[i], target_end_z[i], color=(0.0, 0.0, 0.0), marker="o", s=50)

            #  for i in range(len(plot_optimal_x)):
                #  x_optimal_numpy = np.array(plot_optimal_x[i])
                #  y_optimal_numpy = np.array(plot_optimal_y[i])
                #  z_optimal_numpy = np.array(plot_optimal_z[i])
                
                #  ax.plot_wireframe(x_optimal_numpy, y_optimal_numpy, z_optimal_numpy, color=(0.0 ,0.0 ,1.0))
