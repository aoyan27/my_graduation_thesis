...
from std_msgs.msg import Float64, Int64, Int32, Bool
...
class agent:
    ...
    def button_point_client(self, req_target_point):
        #  rospy.wait_for_service('reward')
        try:
            button_converter = rospy.ServiceProxy('button_point', button_point)
            resp = button_converter(req_target_point)
            return resp.point_x, resp.point_y, resp.point_z
        except rospy.ServiceException, e:
            print "Service call faild : %s" % e
    
    def arm2_start_callback(self, msg):
    ...
    def calculation_path(self):
        ...
        path_point.header.frame_id = "/matching_base_link"
        
        vis_path_point = PointCloud()
        vis_path_point.header.frame_id = "/matching_base_link"
    ...
    def main(self):
        rospy.init_node('q_update_old_method_dxl')
        ...
        rospy.Subscriber("/arm2_start", Bool, self.arm2_start_callback)
        ...
        pub_fin = rospy.Publisher("/arm3_start", Bool, queue_size = 1)
        ...
        self.target_point.header.frame_id = "/matching_base_link"
            ...
            if self.start_flag:
                if self.wait_flag:
                    ...
                    if count == 10:
                        ...
                        pub_5.publish(j5)
                    if count == 100:
                        if self.finish_pub_flag:
                            print "finish_flag publish!!"
                            self.start_flag = False
                            rospy.sleep(1.0)
                            pub_fin.publish(True)
                        else:
                            self.set_speed_client1(0.08)
                    ...
                    if self.button_wait_flag:
                        ...
                        self.target_point.points[0].x -= 0.030
                        self.target_point.points[0].z = 0.94
                        target_x = self.target_point.points[0].x - 0.270 - 0.030
                        target_y = self.target_point.points[0].y - 0.000
                        target_z = self.target_point.points[0].z - 0.935
                        print "traget : ",self.target_point
                        self.next_arm_end_point = self.calculation_path()
                        print "target_x : ", self.target_point.points[0].x
                        print "target_y : ", self.target_point.points[0].y
                        print "target_z : ", self.target_point.points[0].z
                        ...
                        if self.state_observation_flag and self.state_observation_flag1 and self.state_observation_flag2 and self.state_observation_flag3:
                            if self.approach_flag:
                                if i < len(self.next_arm_end_point.points):
                                    ...
                                    print "next_arm_end_x : ", next_arm_end_x + 0.270
                                    print "next_arm_end_y : ", next_arm_end_y + 0.000
                                    print "next_arm_end_z : ", next_arm_end_z + 0.935
                                ...
                                if (joint5_com_limit <= 1 and joint3_com_limit <= 1) and x_addition < 0.010:
