...
int main(int argc, char *argv[]){
	ros::init(argc, argv, "position_dicision2");
	ros::NodeHandle n;
	...
	ros::Publisher pub_2 = n.advertise<std_msgs::Float64>("/joint3_state", 1);
	ros::Publisher pub_3 = n.advertise<std_msgs::Float64>("/joint5_state", 1);
	...
	while(ros::ok()){
		...
		if(pub_flag1 && pub_flag2 && pub_flag3){
			...
			if((fabs(joint_state[0]-joint_com[0])<=1e-3) && 
				...
				pub_2.publish(joint3_state);
				pub_3.publish(joint5_state);
				cout<<"publish !"<<endl;
				// if(count>=20){
					// pub_flag = false;
				// }
