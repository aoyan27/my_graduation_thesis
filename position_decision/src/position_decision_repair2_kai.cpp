...
int main(int argc, char *argv[]){
	...
	init_joint[0] = (double)init_joint1;
	init_joint[1] = (double)init_joint3;
	init_joint[2] = (double)init_joint5;
	...
	while(ros::ok()){
		...
		joint1_state.data = joint_state[0] / pi * 180.0;
		joint3_state.data = joint_state[1] / pi * 180.0;
		joint5_state.data = joint_state[2] / pi * 180.0;
		...
		if(pub_flag1 && pub_flag3 && pub_flag5){
			...
			if((fabs(joint_state[0]-joint_com[0])<=1e-3) && (fabs(joint_state[1]-joint_com[1])<=1e-3) 
				...
				cout<<"num_step : "<<num_step<<endl;
				// cout<<"join1_state - joint1_com = "<<fabs(joint_state[0]-joint_com[0])<<endl;
				cout<<"joint1_state(rad) : "<<joint_state[0]<<endl;
				cout<<"joint3_state(rad) : "<<joint_state[1]<<endl;
				cout<<"joint5_state(rad) : "<<joint_state[2]<<endl;

				cout<<"joint1_state(deg) : "<<joint_state[0]/pi*180.0<<endl;
				cout<<"joint3_state(deg) : "<<joint_state[1]/pi*180.0<<endl;
				cout<<"joint5_state(deg) : "<<joint_state[2]/pi*180.0<<endl;
