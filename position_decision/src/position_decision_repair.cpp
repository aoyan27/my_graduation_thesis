...
void joint4_com_callback(std_msgs::Float64 msg){
	joint_com[3] = msg.data;
	// cout<<"joint1 command : "<<joint_com[0]<<endl;
	pub_flag4 = true;
}
void joint5_com_callback(std_msgs::Float64 msg){
	joint_com[4] = msg.data;
...
int main(int argc, char *argv[]){
	...
	ros::Subscriber sub_2 =n.subscribe("/infant/joint2_position_controller/command", 10, joint2_com_callback); 
	ros::Subscriber sub_3 =n.subscribe("/infant/joint3_position_controller/command", 10, joint3_com_callback); 
	ros::Subscriber sub_4 =n.subscribe("/infant/joint4_position_controller/command", 10, joint4_com_callback); 
	...
	ros::Publisher pub_4 = n.advertise<std_msgs::Float64>("/joint4_state", 1);
	...
	init_joint[3] = (float)init_joint4;
	init_joint[4] = (float)init_joint5;
	...
	joint_com[3] = init_joint[3] / 180.0 * pi;
	joint_com[4] = init_joint[4] / 180.0 * pi;

	std_msgs::Float64 joint1_state;
	std_msgs::Float64 joint2_state;
	std_msgs::Float64 joint3_state;
	std_msgs::Float64 joint4_state;
	...
	while(ros::ok()){
		...
		if(pub_flag1 && pub_flag2 && pub_flag3 && pub_flag5 && pub_flag5){
			...
			if((fabs(joint_state[0]-joint_com[0])<=1e-3) && (fabs(joint_state[1]-joint_com[1])<=1e-3) 
					&& (fabs(joint_state[2]-joint_com[2])<=1e-3) && (fabs(joint_state[3]-joint_com[3])<=1e-3) && (fabs(joint_state[4]-joint_com[4])<=1e-3)){
				cout<<"num_episode : "<<num_episode<<endl;
				cout<<"num_step : "<<num_step<<endl;
				// cout<<"join1_state - joint1_com = "<<fabs(joint_state[0]-joint_com[0])<<endl;
				...
				cout<<"joint4_state : "<<joint_state[3]<<endl;
				cout<<"joint5_state : "<<joint_state[4]<<endl;
				...
				if(joint_state[3]<0.0){
					joint4_state.data = (double)(int)(joint_state[3]/pi * 180.0 - 0.5);
				}
				else{
					joint4_state.data = (double)(int)(joint_state[3]/pi * 180.0 + 0.5);
				}
				if(joint_state[4]<0.0){
					joint5_state.data = (double)(int)(joint_state[4]/pi * 180.0 - 0.5);
				}
				else{
					joint5_state.data = (double)(int)(joint_state[4]/pi * 180.0 + 0.5);
				}
				...
				cout<<"joint4_state(float64_msg) : "<<joint4_state<<endl;
