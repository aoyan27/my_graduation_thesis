...
double	distance_step_0 =celi2(sqrt(pow(target_init[0]-arm_pose_init[0], 2) 
			+ pow(target_init[1]-arm_pose_init[1], 2) + pow(target_init[2]-arm_pose_init[2], 2)), 5);
...
bool reward_server_callback(reward_calculation::reward::Request &req,
	...
	cout<<"arm x : "<<arm_pose[0]<<endl;
	cout<<"arm y : "<<arm_pose[1]<<endl;
	cout<<"arm z : "<<arm_pose[2]<<endl;
	...
	if(req.request_reward == 0){
		...
		distance_step_0 = distance_now;
		cout<<"distance_initialize!!!"<<endl;
	}
	
	cout<<"distance_step_0 : "<<distance_step_0<<endl;
	...
	if((distance_now - distance_past) < 0.0){
		// res.reward = ((distance_step_0 - distance_now) / distance_step_0) * 0.1;
		res.reward = ((distance_past - distance_now) / distance_step_0);
		cout<<"arm approached!! and reward : "<<res.reward<<endl;;
		distance_past = distance_now;
	}
	else if((distance_now - distance_past) > 0.0){
		// res.reward = -0.5;
		res.reward = ((distance_past - distance_now) / distance_step_0);
		cout<<"arm leaved!! and reward : "<<res.reward<<endl;
		distance_past = distance_now;
	}
	else{
		// res.reward = -0.5;
		res.reward = ((distance_past - distance_now) / distance_step_0);
		cout<<"arm doen't move!! and reward : "<<res.reward<<endl;
		distance_past = distance_now;
	}
	cout<<"distance_now : "<<distance_now<<endl;
	cout<<"distance_past : "<<distance_past<<endl;
