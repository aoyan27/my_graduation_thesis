...
int main(int argc, char *argv[]){
	...
	while(ros::ok()){
		...
		if((pub_action_type_flag1 && pub_action_type_flag3 && pub_action_type_flag5) && (pub_pose_com_flag1 && pub_pose_com_flag3 && pub_pose_com_flag5)){
			cout<<"num_step : "<<num_step<<endl;
			// cout<<"limit_joint1 : "<<init_joint[0]<<endl;
			// cout<<"limit_joint3 : "<<init_joint[1]<<endl;
			// cout<<"limit_joint5 : "<<init_joint[2]<<endl;
			...
			printf("joint1_pose.data : %lf\n", joint1_pose.data);
			cout<<"joint1_pose.data : "<<joint1_pose.data<<endl;
			...
			for(i=0;i<num_joint;i++){
				if(action_type[i]==0.0){
					...
					if(i==0){
						if((1.0*(float)fabs(init_joint[0]))<=34.0){
							...
							if(joint_pose[0]>=34.0){
								cout<<"action type : 1.0 but joint_angle laower limit"<<endl;
								joint_pose[i] += 0.0;
							}
							else{
								cout<<"action type : 1.0 -> joint_angle decrease 1 deg"<<endl;
								joint_pose[i] += 1.0;
								// cout<<joint_pose[i]<<endl;
							}	
						}
					...
					if(i==2){
						if(joint_pose[i]>=(float)init_joint[2]){
							cout<<"action type : 1.0 but joint_angle upper limit"<<endl;
							joint_pose[i] += 0.0;
						}
						else{
